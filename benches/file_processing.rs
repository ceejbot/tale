...
use tale_ndjson::readers::{BackSeekingProcessor, BufferedFileProcessor, ChunkedFileReader, FileProcessor};
...
fn bench_file_processors(c: &mut Criterion) {
    ...
    for (size_name, lines) in file_sizes {
        ...
        group.bench_with_input(BenchmarkId::new("buffered", size_name), &test_data, |b, data| {
            b.iter(|| {
                let temp_file = create_test_file(data);
                let mut reader = BufferedFileProcessor::new(temp_file.path()).expect("Failed to create reader");
        ...
        // ChunkedFileReader
        group.bench_with_input(BenchmarkId::new("chunked", size_name), &test_data, |b, data| {
            b.iter(|| {
                let temp_file = create_test_file(data);
        ...
        group.bench_with_input(BenchmarkId::new("backseeking", size_name), &test_data, |b, data| {
            b.iter(|| {
                let temp_file = create_test_file(data);
        ...
        group.bench_with_input(
            ...
            |b, (data, skip)| {
                b.iter(|| {
                    ...
                    reader.skip_lines(*skip as u64).expect("Failed to skip lines");
...
criterion_group!(benches, bench_file_processors, bench_skip_operations);
