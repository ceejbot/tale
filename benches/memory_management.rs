...
fn bench_memory_budget_creation(c: &mut Criterion) {
    ...
    for (name, size) in sizes {
        group.bench_with_input(BenchmarkId::new("new_budget", name), &size, |b, &size| {
            b.iter(|| {
                black_box(MemoryBudget::new(size).expect("Failed to create budget"))
            })
        });
    }
...
fn bench_memory_statistics(c: &mut Criterion) {
    ...
    let _alloc1 = budget.try_allocate(1024 * 1024, "test1")
        .expect("Failed to allocate")
        .expect("Allocation should succeed");
    let _alloc2 = budget.try_allocate(2 * 1024 * 1024, "test2")
        .expect("Failed to allocate")  
        .expect("Allocation should succeed");
